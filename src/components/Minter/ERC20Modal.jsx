...
const ERC20Modal = ({ isAssetModalVisible, handleAssetOk, confirmLoading, handleAssetCancel }) => {
    ...
    return (
        ...
        >
            ...
            <Input style={{ marginBottom: "80px" }} type="number" placeholder="Enter ETH amount" onChange={(e) => setEthAmount(e.target.value)}></Input>
            <Title level={5}>Bundle ERC20 Token</Title>
            <AssetSelector getAsset={onChangeToken} />
            <Input style={{ marginTop: "10px" }} type="number" placeholder="Enter selected token amount" onChange={(e) => onChangeERC20Amount(e.target.value)}></Input>
            <div style={{ width: '100', display: 'flex', justifyContent: 'flex-end' }}>            
                <Button type="primary" style={{ margin: "50px 30px 0px" }} onClick={handleReset} danger>Delete all tokens</Button>
            </div>
        </Modal>
    )
}
